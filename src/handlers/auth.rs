...
pub async fn forgot_password(db: web::Data<Database>, form: web::Json<ForgotPasswordRequest>) -> HttpResponse {
    let collection = db.collection::<User>("users");
    let user = collection.find_one(doc! { "email": &form.email }, None).await.unwrap();
    ...
    if let Some(user) = user {
        ...
        HttpResponse::Ok().json({"message": "If your email is registered with us, you will receive a password reset link."})
    } else {
        HttpResponse::Ok().json({"message": "If your email is registered with us, you will receive a password reset link."})  // Avoid confirming or denying the existence of an account
    }
}
